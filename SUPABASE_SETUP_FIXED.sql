...
CREATE TABLE IF NOT EXISTS users (
  ...
  email VARCHAR(100) UNIQUE NOT NULL,  -- username 대신 email 사용
  password_hash TEXT NOT NULL,          -- password 대신 password_hash
  full_name VARCHAR(100),
  role VARCHAR(20) NOT NULL CHECK (role IN ('admin', 'teacher', 'branch')),  -- korean_branch를 branch로
  agency_name VARCHAR(100),  -- 교사용
  branch_name VARCHAR(100),  -- 한국지점용
...
CREATE TABLE IF NOT EXISTS students (
  ...
  agency_id INTEGER,  -- REFERENCES 제거 (agencies 테이블과 별개로 운영)
